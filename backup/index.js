...
const { help } = require('./olaf/help')
const { insta1 } = require('./src/insta1')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '*⏳ 𝗰𝗮𝗹𝗺𝗮 𝗮í 𝗺𝗮𝗻𝗼 𝘁𝗼𝗺𝗲 𝘂𝗺 𝗰𝗮𝗳é☕𝗲 𝘂𝗺 𝗯𝗶𝘀𝗰𝗼𝗶𝘁𝗼 🥯 𝗽𝗼𝗿 𝗲𝗻𝗾𝘂𝗮𝗻𝘁𝗼...*',					success: '️❬ ✔ ❭ Sucesso 🖤',
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Nesse grupo, não gostamos de link, que isso sirva de exemplo 🚶")
		}, 4000)
	}
	...
	if (messagesC.includes("fdp")){
     	...
     	if (messagesC.includes("olaf?")){
	       ...
	       if (messagesC.includes("sim e vc?")){
			client.updatePresence(from, Presence.composing)
			reply("to bem man⛄🔥")
	}

          if (messagesC.includes("feminino")){
			client.updatePresence(from, Presence.composing)
			reply("perguntei sua sexualidade Não seu gênero")
	}	

			if (messagesC.includes("hetara")){
			client.updatePresence(from, Presence.composing)
			reply("responde certo por favor")
	}	
...
//msc
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
						...
						if (messagesC.includes("suiton")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/suiton.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			  ...
			  if (messagesC.includes("clbc")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/clbc.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
		...
		case 'iri':
			...
			break
				...
				decbin = `${body.slice(11)}`
					anu = await fetchJson(`https://api.anoncybfake+er.com/api/binary/?decode=${decbin}`, {method: 'get'})
					frhan.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					await limitAdd(sender) 
					break  
				case 'encoctal':
		...
		case 'bpfont':
          ...
          case 'letrasmsc':
			...
			case 'husbu':
				...
				break
                case 'teste':
				 data = fs.readFileSync('./src/teste.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
...
break
case 'say6':
tujuh = fs.readFileSync('./assets/say6.mp4');
client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
//xxx
case 'xxx1':
...
//variats
case 'ajuda':
tujuh = fs.readFileSync('./assets/ajuda.mp4');
client.sendMessage(from, tujuh, video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
           ...
           case 'corno':
              client.updatePresence(from, Presence.composing) 
                random = `${Math.floor(Math.random() * 100)}`
               hasil = `Nivel de cm vc é corno\n\nVocê é: *${random}%* CORNO(A)😂`
              reply(hasil)
                break
           case 'iludido':
              client.updatePresence(from, Presence.composing) 
                random = `${Math.floor(Math.random() * 100)}`
               hasil = `Nivel de quanto vc é iludido\n\nVocê é: *${random}%* iludido(A)💔🔥`
              reply(hasil)
                break
           case 'golpe':
              client.updatePresence(from, Presence.composing) 
                random = `${Math.floor(Math.random() * 100)}`
               hasil = `Nivel de quanto vc da golpe\n\nVocê da: *${random}%* golpe😏🔥`
              reply(hasil)
                break
                case 'lgbt':
              client.updatePresence(from, Presence.composing) 
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
				...
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior   do texto é merda')
		   ...
		   case 'perfil':		  		           
				...
				case 'perguntas':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQJp_DC3vPoV_TlFGPv9V2ogeHdM5ulKPQTYg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'então se vc quiser jogar esse jogo vai funcionar assim cm N tem garrafa kkkk vamos usar o dado\n\ncomo vamos usar o dado se vc tira\n5,10,3 vai ter que responder ou fazer desafio pra jogar use .jogar⛄️\n\n BOA SORTE'})
				case 'hentai':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Aleh⃖ ☔\n*YOUTUBE:* sem\n*WPP:* wa.me/5577991174304\n*INSTA:* sem\n\n\nEspero que tenham gostado do bot ⛄🔥️'})
					break
				case 'insta':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://uploaddeimagens.com.br/images/003/097/994/original/20210223_214950.jpg?1614129156`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'opan, dá insta1 pra ver as divulgações'})
					break
				case 'nanatsu1':
			     	memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSANY7ag-mjxWRUyeG_7JAFEB4n-2owq0wTpA&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'animes hehehe'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
